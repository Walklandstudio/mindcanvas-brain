 ...
 *
 *  weightsX are JSON strings (e.g. {"P1":2,"P4":1})
 */
...
/** Minimal CSV splitter that supports commas inside quoted fields */
  ...
  if (qRows.length && testRows.length) {
    ...
    for (const r of qRows) {
      ...
      for (const o of opts) {
        ...
        try {
          weights = o.weights ? (JSON.parse(o.weights) as Record<string, number>) : {};
        } catch {
          throw new Error(
            `Invalid JSON in weights for idx=${idx}, option=${o.code}`
          );
        }
