...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const first_name = (body.first_name ?? null) as string | null;
  const last_name = (body.last_name ?? null) as string | null;
  const phone = (body.phone ?? null) as string | null;

  // latest taker for this token (created by /start)
  ...
  return res.status(200).json({ ok: true, taker_id: up.data?.id ?? null });
}
