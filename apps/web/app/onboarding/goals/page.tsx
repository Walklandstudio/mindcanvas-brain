...
type Goals = {
  ...
  integration?: string;              // integration selection
...
const SECTORS = ['B2B','B2C','Public Sector','Enterprise','SMB / Mid-market','Startups','Other'];

const INTEGRATIONS = [
  'Email only','Embedded on website','CRM (HubSpot / Salesforce)','LMS / LXP',
  'HRIS / ATS','Zapier / Webhooks','Custom API',
];

export default function Page() {
  const { data, update, saving, saveNow, loadFromServer, clearDraft } =
    useOnboardingAutosave<Goals>('goals', {});

  useEffect(() => {
    (async () => {
      const r = await fetch('/api/onboarding', { cache: 'no-store' });
      const j = await r.json();
      loadFromServer(j.onboarding?.goals ?? {});
    })();
  }, [loadFromServer]);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <Area label="What is the primary goal of the profile test?"
            value={data.primaryGoal ?? ''} onChange={v => update('primaryGoal', v)} />
      <Area label="How does this test align with your company’s mission or vision?"
            value={data.visionAlign ?? ''} onChange={v => update('visionAlign', v)} />
      <Area label="What specific outcomes would you like participants to achieve?"
            value={data.outcomes ?? ''} onChange={v => update('outcomes', v)} />
      <Area label="Who will primarily take this test?"
            value={data.audience ?? ''} onChange={v => update('audience', v)} />
      <Area label="Are there any challenges your audience faces that the test could help address?"
            value={data.challenges ?? ''} onChange={v => update('challenges', v)} />
      <Area label="Other insights you want to collect as part of your questions?"
            value={data.extraInsights ?? ''} onChange={v => update('extraInsights', v)} />
      <Area label="Industry-relevant info (revenue, targets, etc.)"
            value={data.industryInfo ?? ''} onChange={v => update('industryInfo', v)} />

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label className="block text-sm mb-1">Program Type</label>
          <select className="w-full rounded-md border px-3 py-2"
      ...
      <div className="flex gap-3">
        ...
        <button
          onClick={async () => { await saveNow(); clearDraft(); window.location.assign('/admin/framework'); }}
          disabled={saving}
