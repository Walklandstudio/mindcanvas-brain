...
export async function suggestFrameworkNames(input: {
  ...
  const fallback = {
    frequencies: { A: "Vision & Innovation", B: "Connection & Influence", C: "Grounding & Timing", D: "Precision & Systems" },
    profiles: [
...
Frequencies (A–D) must be professional and brand-aligned.
Profiles (8) are 2 per frequency, unique, <=4 words.
Return JSON with keys: frequencies, profiles, imagePrompts.`;
...
- Exactly 8 profiles total (2 per A/B/C/D).`;
