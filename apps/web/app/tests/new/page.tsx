...
export default function NewTestPage({ searchParams }: { searchParams: { mode?: string } }) {
  const initialMode = (searchParams?.mode === 'free' || searchParams?.mode === 'full') ? searchParams.mode : 'full';
      ...
      <div className="mt-6 space-y-4">
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-3">
            <label className="flex items-center gap-2 text-sm"><input type="radio" name="mode" checked={mode==='free'} onChange={()=>setMode('free')} /> Free</label>
            <label className="flex items-center gap-2 text-sm"><input type="radio" name="mode" checked={mode==='full'} onChange={()=>setMode('full')} /> Full</label>
          </div>
          <p className="text-xs text-gray-500 mt-1">{help}</p>
        </div>
        <button onClick={create} disabled={creating} className="px-4 py-2 rounded-xl bg-black text-white">{creating?'Creating…':'Create Test & Link'}</button>
        ...
        {share && (
          <div className="mt-6 rounded-xl border p-4">
            ...
            <div className="mt-4 flex gap-3">
              <a className="px-3 py-2 rounded-xl border" href={share.shareUrl} target="_blank">Open Test</a>
              <a className="px-3 py-2 rounded-xl border" href={`${share.shareUrl}/result`} target="_blank">View Result</a>
              <a className="px-3 py-2 rounded-xl border" href={`${share.shareUrl}/report`} target="_blank">View Report</a>
            </div>
          </div>
        )}
      </div>
