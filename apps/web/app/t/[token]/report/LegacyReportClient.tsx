...
// ---------------- Legacy (Team Puzzle / Competency Coach) helpers ----------------
...
export default function LegacyReportClient(props: { token: string; tid: string }) {
  ...
  // IMPORTANT: page H1 is ALWAYS the test name (keeps LEAD consistent with Team Puzzle)
  const pageTitle = data.test_name || `${orgName} Report`;

  // ---------------- RENDER SHELL (same for all reports) ----------------
        ...
        <div className="mx-auto flex max-w-5xl flex-col gap-8 px-4 pb-12 pt-8 md:px-6">
          ...
          {useSections ? (
            <section className="space-y-6">
              ...
              {/* Keep the LEAD-specific report title INSIDE the report (NOT as page H1) */}
              ...
              {/* Profile missing warning (only if framework doesn’t include that profile) */}
            ...
            <LegacyFrameworkReport data={data} />
          )}
...
// ---------------- Full legacy renderer (restores the “great” report) ----------------

function LegacyFrameworkReport({ data }: { data: ResultData }) {
  ...
  return (
    <>
      {/* HEADER-LIKE TITLE CARD FOR LEGACY REPORT BODY */}
      <section className="space-y-6">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-300">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 md:p-7 text-slate-900">
          ...
          <h3 className="mt-4 text-base font-semibold text-slate-900">{welcomeTitle}</h3>
              ...
              >
                ...
                <ul className="mt-3 flex-1 list-disc space-y-1 pl-4 text-xs text-slate-700">
                  ...
                  <li>
                    ...
                    {asText(copy?.motivators) || "Conditions that help this style feel energising."}
                  </li>
                  <li>
                    <span className="font-semibold">Watch outs:</span>{" "}
                    {asText(copy?.blind_spots) || "Things to watch out for under pressure."}
                  </li>
                </ul>
              </div>
            );
