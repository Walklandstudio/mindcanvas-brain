...
export default function DashboardClient(props: {
  ...
  const profData = (props.profiles ?? [])
    .map(r => ({ name: r.profile_name, points: Number(r.avg_points), code: r.profile_code }))
  ...
  return (
    <div className="p-6 grid grid-cols-12 gap-6">
      ...
      <Card className="col-span-12 lg:col-span-8">
        <CardHeader><CardTitle>Frequency Mix</CardTitle></CardHeader>
      ...
      {/* Profiles */}
      <Card className="col-span-12">
        <CardHeader><CardTitle>Profile Mix</CardTitle></CardHeader>
          ...
          <ul className="space-y-2">
            {(props.top3 ?? []).map((t, i) => (
              <li key={`${t.profile_code}-${i}`} className="flex items-center justify-between">
                ...
                <span className="text-emerald-600">{Number((t as any).avg_points)}</span>
              </li>
            ))}
          </ul>
        </CardContent>
        ...
        <CardContent>
          <ul className="space-y-2">
            {(props.low3 ?? []).map((t, i) => (
              <li key={`${t.profile_code}-${i}`} className="flex items-center justify-between">
                <span className="font-medium">{t.profile_name}</span>
                <span className="text-rose-600">{Number((t as any).avg_points)}</span>
              </li>
            ))}
          </ul>
        </CardContent>
