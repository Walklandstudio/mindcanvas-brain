...
export default async function TestsPage({ params }: { params: { slug: string } }) {
  try {
    const res = await fetch(`/api/org/${params.slug}/tests`, { cache: 'no-store' });
    ...
    if (!res.ok) {
      ...
      return (
        <div className="p-6 text-red-300">
          Failed to load tests (HTTP {res.status})
        </div>
      );
    }
    ...
    return (
      <div className="p-6 text-red-300">
        Server error while rendering Tests page: {String(e?.message ?? e)}
      </div>
    );
  }
}
