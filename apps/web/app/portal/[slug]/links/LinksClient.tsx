        ...
        <div className="space-y-3 rounded-xl border p-4 bg-white">
          <label className="text-sm block">
            ...
            <select className="w-full rounded border p-2" value={testId} onChange={(e) => setTestId(e.target.value)}>
              ...
              {tests.map(t => <option key={t.id} value={t.id}>{t.name}{t.test_type ? \` (\${t.test_type})\` : ""}</option>)}
            </select>
          </label>

          <label className="text-sm block">
            <span className="mb-1 block">Name Test</span>
            <input className="w-full rounded border p-2" value={testDisplayName} onChange={(e) => setTestDisplayName(e.target.value)} placeholder="e.g. Team Puzzle Profile — Sales intake" />
          </label>

          <label className="text-sm block">
            <span className="mb-1 block">Contact owner&apos;s name</span>
            <input className="w-full rounded border p-2" value={contactOwner} onChange={(e) => setContactOwner(e.target.value)} placeholder="e.g. Sarah Ndlovu" />
          </label>
          ...
          {!showResults && (
            <div className="space-y-3">
              <label className="block text-sm">
                <span className="mb-1 block">Thank you / next steps message (shown after completion)</span>
                <textarea rows={4} className="w-full rounded border p-2" value={message} onChange={(e) => setMessage(e.target.value)} />
              </label>
              <label className="block text-sm">
                <span className="mb-1 block">Redirect URL (optional)</span>
                <input className="w-full rounded border p-2" value={redirectUrl} onChange={(e) => setRedirectUrl(e.target.value)} placeholder="https://example.com/next-steps" />
              </label>
            </div>
          )}
          ...
          <label className="text-sm block">
            ...
            <input type="datetime-local" className="w-full rounded border p-2" value={expiresAt} onChange={(e) => setExpiresAt(e.target.value)} />
            <span className="block mt-1 text-xs text-gray-500">Leave blank to never expire.</span>
          </label>

          <div className="flex items-center gap-3 pt-1">
            <button className="rounded bg-black px-3 py-2 text-white text-sm disabled:opacity-50" disabled={!testId || loading} onClick={generate}>
                ...
                {links.map(r => {
                  ...
                  return (
                    <tr key={r.token} className="border-t">
                      <td className="px-3 py-2">{r.test_name}{r.contact_owner && <span className="ml-2 text-xs text-gray-500">({r.contact_owner})</span>}</td>
                      <td className="px-3 py-2">{r.created_at ? new Date(r.created_at).toLocaleString() : "—"}{!r.is_active && <span className="ml-2 text-xs text-gray-500">(inactive)</span>}</td>
                      <td className="px-3 py-2">{r.show_results ? "Shown" : "Hidden"}</td>
                      <td className="px-3 py-2">{r.expires_at ? \`\${new Date(r.expires_at).toLocaleString()}\${expired ? " (expired)" : ""}\` : "—"}</td>
                      <td className="px-3 py-2">
                        <button type="button" onClick={() => doCopy(url)} className="underline" title="Copy link">/t/{r.token}</button>
                      </td>
                    </tr>
                  );
                })}
                {!links.length && <tr><td className="px-3 py-6 text-center text-gray-500" colSpan={5}>No links yet.</td></tr>}
              </tbody>
            </table>
          </div>
