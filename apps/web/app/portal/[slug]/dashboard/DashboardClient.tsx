...
export default function DashboardClient(props: { orgSlug: string }) {
  const org = props.orgSlug;
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        if (!active) return;
        if (!json || json.ok !== true) setError((json && json.error) || "Unknown error");
  ...
  function dlFreq() {
    downloadCSV("frequencies_" + org + ".csv", freq);
  }
  function dlProf() {
    downloadCSV("profiles_" + org + ".csv", prof);
  }
    ...
    <div className="space-y-8">
      ...
      {overall && (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <div className="rounded-2xl border p-4">
            ...
            <div className="text-2xl font-semibold">{org}</div>
          </div>
        </div>
      )}
      ...
      <section className="rounded-2xl border p-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <button className="rounded-md border px-3 py-1.5 text-sm" disabled={freq.length === 0} onClick={dlFreq}>
            Download CSV
          </button>
        </div>
      ...
      <section className="rounded-2xl border p-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <button className="rounded-md border px-3 py-1.5 text-sm" disabled={prof.length === 0} onClick={dlProf}>
            Download CSV
          </button>
        </div>
        <div className="h-64 w-full">
