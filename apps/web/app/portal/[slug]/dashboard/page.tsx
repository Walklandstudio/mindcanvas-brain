...
export default async function Page({ params }: { params: { slug: string; testId: string } }) {
  ...
  const { data: org } = await db.from("portal.v_organizations").select("id, slug, name").eq("slug", params.slug).maybeSingle();
  if (!org) return <div className="p-6 text-red-600">Org not found</div>;
  const { data: test } = await db.from("portal.v_org_tests").select("*").eq("id", params.testId).maybeSingle();
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {wrongOrg && <div className="rounded border border-amber-300 bg-amber-50 p-3 text-amber-800 text-sm">
