...
type Sections = { strengths?: string; challenges?: string; roles?: string; guidance?: string; visibility?: string };

export default function ClientEditor({ profiles, drafts }:{
  profiles: Profile[]; drafts: Record<string, Sections>;
}) {
  const [active, setActive] = useState(profiles[0]?.id);
  const [sections, setSections] = useState<Record<string, Sections>>({ ...drafts });

  useEffect(() => { profiles.forEach(p => { if (!sections[p.id]) sections[p.id]={}; }); }, []); // ensure keys
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-6">
      ...
      <section className="space-y-5">
        ...
        {(['strengths','challenges','roles','guidance','visibility'] as (keyof Sections)[])
          .map(k => (
            <div key={k}>
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-4" style={vars}>
          <h4 className="text-base font-semibold" style={{ color:'var(--brand-accent)', fontFamily:'var(--brand-font)'}}>
            {prof?.name} — Report Preview
          </h4>
