...
export default function ReportEditorPage({ params }: { params: { id: string } }) {
  ...
  async function draftAI() {
    setBusy("draft");
    try {
      const res = await fetch(`/api/admin/reports/${params.id}?action=draft`, { method: "POST" });
      const j = await res.json();
  ...
  async function signoff() {
    setBusy("signoff");
    try {
      const res = await fetch(`/api/admin/reports/${params.id}?action=signoff`, { method: "POST" });
      const j = await res.json();
  ...
  return (
    <main className="max-w-6xl mx-auto p-6 text-white">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            onClick={draftAI}
            disabled={busy === "draft"}
            className="px-3 py-2 rounded-xl bg-white/10 border border-white/15 hover:bg-white/15 disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
        <Section
          title="Strengths"
          value={data.report.strengths}
          onChange={(v) => setData({ ...data, report: { ...data.report, strengths: v } })}
        />
        <Section
          title="Challenges"
          value={data.report.challenges}
          onChange={(v) => setData({ ...data, report: { ...data.report, challenges: v } })}
        />
        <Section
          title="Ideal Roles"
          value={data.report.roles}
          onChange={(v) => setData({ ...data, report: { ...data.report, roles: v } })}
        />
        <Section
          title="Guidance"
          value={data.report.guidance}
          onChange={(v) => setData({ ...data, report: { ...data.report, guidance: v } })}
        />
      </div>
...
function Section({ title, value, onChange }: { title: string; value: string; onChange: (v: string) => void }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      ...
      <textarea
        className="w-full h-56 rounded-xl bg-black/20 border border-white/10 p-3 outline-none"
        value={value}
