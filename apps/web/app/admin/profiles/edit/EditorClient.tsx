...
function coerceFreq(v: string | null | undefined): Freq {
  const up = (v ?? 'A').toUpperCase();
  return (['A', 'B', 'C', 'D'] as const).includes(up as Freq) ? (up as Freq) : 'A';
}
...
export default function EditorClient() {
  ...
  const [frequency, setFrequency] = useState<Freq>(initialFreq);
  ...
  return (
    <main className="max-w-5xl mx-auto p-6 text-white">
      ...
      <p className="text-white/70 mt-2">Generate and edit content for the selected profile.</p>
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <label className="block">
          ...
          <input
            className="w-full rounded-xl bg-white text-black p-3"
            value={frequency}
            onChange={(e) => setFrequency(coerceFreq(e.target.value))}
            placeholder="A / B / C / D"
          />
        </label>
      </div>
  ...
  return (
    <section className="mt-10 space-y-8">
      ...
      <div className="pt-4 flex gap-3">
        ...
        <button className="px-5 py-2 rounded-xl bg-white/10" onClick={() => router.push('/admin/framework')}>
          Back to Framework
        </button>
      </div>
    </section>
