...
export async function GET(req: Request) {
  try {
    ...
    const { data, error } = await sb.from("profiles_drafts").select("*").eq("id", id).single();
...
export async function PATCH(req: Request) {
  try {
    ...
    const sb: any = supabaseAdmin();
    const { data, error } = await sb.from("profiles_drafts").update(update).eq("id", id).select("*").single();
    if (error) throw error;
    return NextResponse.json({ ok: true, draft: data });
  } catch (e: any) {
