...
function toZeroBasedSelected(row: any): number | null {
  ...
  if (typeof row.selected_index === "number")
    return row.selected_index;
  if (row?.value && typeof row.value.index === "number")
...
export async function POST(
  ...
  try {
    ...
    const answers: any[] = Array.isArray(body.answers)
      ? body.answers
      : [];
    const sb = supa();
    ...
    const { data: taker, error: takerErr } = await sb
      .from("test_takers")
      .select(
        "id, test_id, link_token, first_name, last_name, email, company, role_title"
      )
      .eq("id", takerId)
      .eq("link_token", token)
      .maybeSingle();
    ...
    const isQscTest =
      ...
      kindLower === "qsc" ||
      resultTypeLower === "qsc" ||
      ["entrepreneur", "leaders"].includes(qscVariantLower);

    const isQscEntrepreneur =
      isQscTest &&
      (qscVariantLower === "entrepreneur" ||
        slugLower.includes("core"));
    ...
    if (subErr) {
      return NextResponse.json(
        {
          ok: false,
          error: `Submission insert failed: ${subErr.message}`,
        },
        { status: 500 }
      );
    }
    ...
    if (upErr) {
      return NextResponse.json(
        {
          ok: false,
          error: `Results upsert failed: ${upErr.message}`,
        },
        { status: 500 }
      );
    }
    ...
    if (isQscTest) {
      try {
        ...
        const answersForScoring = answers
          ...
          .filter(
            (a: any) => a.question_id && a.choice >= 0
          );
        ...
        if (scoring.combinedProfileCode) {
          ...
          if (personality_code && mindset_level) {
            const { data: qscProfileRow, error: qscProfileError } =
              await sb
                .from("qsc_profiles")
                .select("id")
            ...
            if (qscProfileError) {
              console.error(
                "QSC scoring: failed to load qsc_profile row",
                qscProfileError
              );
            } else {
        ...
        // 3) INSERT into portal.qsc_results (one row per submission)
        const { error: qscInsertError } = await sb
          .from("qsc_results")
          .insert({
            test_id: taker.test_id,
            token, // use test link token (e.g. 4zhyUoRcUd9B)

            personality_totals: scoring.personalityTotals,
            personality_percentages:
              scoring.personalityPercentages,
        ...
        if (qscInsertError) {
          console.error(
            "QSC scoring: failed to insert qsc_results",
            qscInsertError
          );
        }
      } catch (e) {
    ...
    // Compute redirect URL for QSC Entrepreneur – Strategic Growth Report
