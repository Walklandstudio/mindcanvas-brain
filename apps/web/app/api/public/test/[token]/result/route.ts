...
export async function GET(
  ...
  // 1) link -> test_id + link behaviour flags
  const link = await sb
    .from("test_links")
    .select("test_id, show_results, redirect_url, hidden_results_message, next_steps_url")
    .eq("token", token)
    .maybeSingle();
  ...
  if (!link.data?.test_id)
    return NextResponse.json({ ok: false, error: "test not found for token" }, { status: 404 });
  ...
  const linkMeta = {
    ...
    hidden_results_message: (link.data.hidden_results_message as string | null) ?? null,
  ...
  if (fl.error)
    return NextResponse.json({ ok: false, error: fl.error.message }, { status: 500 });
  if (!fl.data?.length)
    return NextResponse.json(
  ...
  if (pl.error)
    return NextResponse.json({ ok: false, error: pl.error.message }, { status: 500 });
  if (!pl.data?.length)
    return NextResponse.json(
  ...
  if (zeroTotals(frequencyTotals, profileTotals) && rawAnswers.length > 0) {
    ...
    if (qs.error)
      return NextResponse.json({ ok: false, error: qs.error.message }, { status: 500 });
