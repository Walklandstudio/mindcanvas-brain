...
// NOTE: do not type the 2nd arg in Next 15
export async function POST(req: Request, context: any) {
  try {
    ...
    // Your schema requires test_takers.token (NOT NULL)
    const takerToken = makeTakerToken();

    // Minimal insert to match your table (no role/phone/team/link_id, etc.)
    const ins = await sb
      .from('test_takers')
      .insert([
        {
          ...
          token: takerToken,           // ✅ satisfy NOT NULL token
          email: body.email ?? null,
          first_name: body.firstName ?? null,
          last_name: body.lastName ?? null,
          // source / link_id omitted because your table doesn't have them
        } as any,
      ])
      .select('id')
      .maybeSingle();
    ...
    return NextResponse.json({ ok: true, takerId, takerToken }, { status: 200 });
  } catch (e: any) {
