...
export async function POST(req: Request, { params }: { params: { token: string } }) {
  ...
  try {
    ...
    const first_name = norm(body.first_name ?? null);
    const last_name  = norm(body.last_name ?? null);
    const email      = norm(body.email ?? null);
    const company    = norm(body.company ?? null);
    const role_title = norm(body.role_title ?? null);

    // Resolve token -> test/org
    ...
    // Create taker (minimal if no details)
    ...
    return NextResponse.json(
      { ok: true, taker_id: taker.id, test_id: link.test_id },
      { status: 200 }
    );
  } catch (e: any) {
