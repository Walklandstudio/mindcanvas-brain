...
export async function GET(
  ...
  if (resultError) {
    console.error("[QSC result] error loading qsc_results:", resultError);
  ...
  if (result.qsc_profile_id) {
    ...
    if (error) {
      console.error("[QSC result] error loading qsc_profile by id:", error);
    } else {
      profile = data;
    }
  }
  ...
  if (profile?.personality_code && profile?.mindset_level) {
    ...
    if (ownerError) {
      console.error(
        "[QSC result] error loading entrepreneur_owner_insights:",
        ownerError
      );
